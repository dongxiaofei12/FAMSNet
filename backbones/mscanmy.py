...
from mmseg.models.backbones.convnext_wtin import wtinblock, attblock
...
class MSCANMy(BaseModule):
    ...
    def __init__(self,
        ...
        for i in range(num_stages):
            ...
            cur += depths[i]
            if i < 2:
                self.wtin.append(wtinblock(embed_dims[i],norm_cfg=dict(type='LN2d', eps=1e-6),act_cfg=dict(type='GELU'),num_heads=2 * (i+1), dpr=0.1 * i))
            else:
                self.att.append(attblock(embed_dims[i],norm_cfg=dict(type='LN2d', eps=1e-6),act_cfg=dict(type='GELU'),num_heads=2 ** (i+1), dpr=0.1 * i))
