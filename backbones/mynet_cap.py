...
from mmseg.models.utils import build_norm_layer, GRN
...
class DenseCapsule(nn.Module):
    ...
    def forward(self, x):
        # 计算 u_hat: [batch_size, out_num_caps, in_num_caps, out_dim_caps]
        u_hat = torch.matmul(self.weight, x[:, None, :, :, None]).squeeze(-1)

        # 分离梯度，不在反向传播中计算 u_hat 的梯度
        u_hat_detached = u_hat.detach()

        # 初始化 b: [batch_size, out_num_caps, in_num_caps]
        b = torch.zeros(x.size(0), self.out_num_caps, self.in_num_caps, device=x.device)

        # 路由算法
        for i in range(self.routings):
            c = F.softmax(b, dim=1)  # Softmax 在 out_num_caps 维度上
            if i == self.routings - 1:
                # 最后一次迭代使用 u_hat
                v = squash(torch.sum(c[:, :, :, None] * u_hat, dim=-2, keepdim=True))
            else:
                # 其他迭代使用 u_hat_detached
                v = squash(torch.sum(c[:, :, :, None] * u_hat_detached, dim=-2, keepdim=True))
                b = b + torch.sum(v * u_hat_detached, dim=-1)

        return torch.squeeze(v, dim=-2)  # [batch_size, out_num_caps, out_dim_caps]


class CapsuleNet(nn.Module):
    def __init__(self, channels, classes, routings):
        super(CapsuleNet, self).__init__()
        self.channels = channels  # (channels, height, width)
        self.classes = classes
        self.routings = routings
        self.conv1 = nn.Conv2d(channels, 256, kernel_size=9, stride=1, padding=0)
        self.primarycaps = PrimaryCapsule(256, 256, 8, kernel_size=9, stride=2, padding=0)
        self.pool = nn.AdaptiveAvgPool2d((28, 28))

        # 固定 h2 和 w2 为 6，因为经过 conv1 和 primarycaps 后特征图尺寸为 6x6
        self.h2, self.w2 = 6, 6
        # 计算主胶囊数量
        self.num_primary_caps = (256 // 8) * self.h2 * self.w2  # (out_channels / dim_caps) * h2 * w2

        self.digitcaps = DenseCapsule(
            in_num_caps=self.num_primary_caps,
            in_dim_caps=8,
    ...
    def forward(self, x, y=None):
        # 计算经过 conv1 和 primarycaps 后的特征图尺寸
        h, w = x.shape[2:]

        x = self.pool(x)
        x = self.relu(self.conv1(x))  # [batch_size, 256, h1, w1]
        x = self.primarycaps(x)  # [batch_size, num_primary_caps, 8]
        x = self.digitcaps(x)  # [batch_size, num_classes, 16]

        length = x.norm(dim=-1)  # [batch_size, num_classes]
        if y is None:
            index = length.max(dim=1)[1]
            y = torch.zeros(length.size(), device=x.device).scatter_(1, index.view(-1, 1), 1.)
        reconstruction = self.decoder((x * y[:, :, None]).view(x.size(0), -1))
        reconstruction = reconstruction.view(-1, self.channels, 28, 28)
        reconstruction = F.interpolate(reconstruction, size=(h, w), mode='bilinear', align_corners=True)  # [batch_size, channels, h, w]
...
class MyNetCAP(BaseModule):
    ...
    def __init__(self,
        ...
        for i in range(self.num_stages):
            ...
            if i==2:
                ...
                stage = nn.Sequential(
                    CapsuleNet(channels=channels, classes=5, routings=3),
                )
                block_idx += depth
    ...
    def train(self, mode=True):
        super(MyNetCAP, self).train(mode)
        self._freeze_stages()
