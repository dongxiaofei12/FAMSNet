...
from .convnext_wtin import attblock, wtinblock
...
class ResNetMy(BaseModule):
    ...
    def __init__(self,
        ...
        for i, num_blocks in enumerate(self.stage_blocks):
            ...
            if i < 2:
                self.wtin.append(wtinblock(self.inplanes,norm_cfg=dict(type='LN2d', eps=1e-6),act_cfg=dict(type='GELU'),num_heads=2 * (i+1), dpr=0.1 * i))
            else:
                self.att.append(attblock(self.inplanes,norm_cfg=dict(type='LN2d', eps=1e-6),act_cfg=dict(type='GELU'),num_heads=2 ** (i+1), dpr=0.1 * i))
        self._freeze_stages()
