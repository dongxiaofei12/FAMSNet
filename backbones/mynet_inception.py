...
class InceptionDWConv2d(nn.Module):
    ...
    def __init__(self, in_channels, square_kernel_size=3, band_kernel_size=11, branch_ratio=0.125,drop_path_rate=0.2,
                 norm_cfg=dict(type='LN2d', eps=1e-6),
                 act_cfg=dict(type='GELU'),):
        super().__init__()

        gc = int(in_channels * branch_ratio)  # channel numbers of a convolution branch
        self.norm_hw = build_norm_layer(norm_cfg, gc)
        self.norm_w = build_norm_layer(norm_cfg, gc)
        self.norm_h = build_norm_layer(norm_cfg, gc)
...
class ConvNeXtBlock(BaseModule):
    ...
    def __init__(self,
                 ...
                 inception_conv_cfg=dict(square_kernel_size=3, band_kernel_size=11, branch_ratio=0.125),
                 norm_cfg=dict(type='LN2d', eps=1e-6),
                 act_cfg=dict(type='GELU'),
                 mlp_ratio=4.,
                 linear_pw_conv=True,
        ...
        if use_inception:
            self.inception_conv = InceptionDWConv2d(in_channels, **inception_conv_cfg)
        else:
...
class MyNetInception(BaseModule):
    ...
    def train(self, mode=True):
        super(MyNetInception, self).train(mode)
        self._freeze_stages()
