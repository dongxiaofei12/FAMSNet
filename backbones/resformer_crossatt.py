...
from .buildformer import Stem, StageModule
...
class FeatureFusionModule(nn.Module):
    ...
    def forward(self, conv_feat, trans_feat):
...
class CrossAttentionModule(nn.Module):
    def __init__(self, in_channels, embed_dim, num_heads=8, dropout=0.0):
    ...
    def forward(self, query, key_value, pad_mask=None):
        """
        query: ResNet 特征 (batch_size, channels, height, width)
        key_value: BuildFormer 特征 (batch_size, channels, height, width)
        pad_mask: [batch_size, seq_len], optional
        """
        batch_size, channels, height, width = query.size()
        N = height * width

        # 通过卷积获取 Query, Key 和 Value
        Q = self.query_conv(query).reshape(batch_size, channels, -1).permute(0, 2, 1)  # [B, N, C]
        K = self.key_conv(key_value).reshape(batch_size, channels, -1).permute(0, 2, 1)  # [B, N, C]
        V = self.value_conv(key_value).reshape(batch_size, channels, -1).permute(0, 2, 1)  # [B, N, C]

        Q = self.layer_norm(Q)
        K = self.layer_norm(K)
        V = self.layer_norm(V)
        # 使用 nn.MultiheadAttention
        attn_output, attn_weights = self.attention(Q, K, V, key_padding_mask=pad_mask)  # [B, N, C]

        # 将输出重新排列为 [B, C, H, W]
        attn_output = attn_output.permute(0, 2, 1).reshape(batch_size, channels, height, width)

        # 最终的输出卷积
        out = self.fc_out(attn_output)  # [B, in_channels, H, W]
...
class ResFormerCrossAtt(BaseModule):
    """ResNet backbone.
    ...
    def __init__(self,
        ...
        for channels in dims:
            self.fusion_modules.append(CrossAttentionModule(channels, channels))
    ...
    def forward(self, x):
        ...
        former_features = []
        former_x = self.stem(former_x)
        former_x = self.stage1(former_x)
        assert not torch.isnan(x).any(), "Backbone output contains NaN"
        former_features.append(former_x)
        former_x = self.stage2(former_x)
        assert not torch.isnan(former_x).any(), "Backbone output contains NaN"
        former_features.append(former_x)
        former_x = self.stage3(former_x)
        assert not torch.isnan(former_x).any(), "Backbone output contains NaN"
        former_features.append(former_x)
        former_x = self.stage4(former_x)
        assert not torch.isnan(former_x).any(), "Backbone output contains NaN"
        former_features.append(former_x)
        fused_features = []
        for i in range(len(res_features)):
            fused = self.fusion_modules[i](res_features[i], former_features[i])
