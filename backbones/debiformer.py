...
class DeBiLevelRoutingAttention(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, n_win=7, qk_dim=None, qk_scale=None,
        ...
        self.conv_offset_q = nn.Sequential(
            nn.Conv2d(self.n_group_channels, self.n_group_channels, (self.kk, self.kk),
                      (self.stride_def, self.stride_def), (self.kk // 2, self.kk // 2), groups=self.n_group_channels,
                      bias=False),
    ...
    def forward(self, x, ret_attn_mask=False):
        ...
        ##################side_dwconv(lepe)##################
        # NOTE: call contiguous to avoid gradient warning when using ddp
        lepe1 = self.lepe1(rearrange(kv[..., self.qk_dim:], 'n (j i) h w c -> n c (j h) (i w)', j=self.n_win,
                                     i=self.n_win).contiguous())
        ...
        queries_def = rearrange(queries_def, "n c (j h) (i w) -> n (j i) h w c", j=self.n_win,
                                i=self.n_win).contiguous()
        ...
        out_def = rearrange(out, '(n j i) m (h w) c -> n (m c) (j h) (i w)', j=self.n_win, i=self.n_win,
                            h=Hg // self.n_win, w=Wg // self.n_win).contiguous()
        ...
        displacement = (
                    q_grid.reshape(N * self.n_groups, H * W, 2).unsqueeze(2) - pos_k.reshape(N * self.n_groups, Hg * Wg,
    ...
    def __init__(self, dim, drop_path=0., layer_scale_init_value=-1,
        ...
        if topk == 4:
            ...
            self.attn1 = DeBiLevelRoutingAttention(dim=dim, num_heads=num_heads, n_win=n_win, qk_dim=qk_dim,
                                                   ...
                                                   kv_downsample_kernel=kv_downsample_kernel,
                                                   kv_downsample_mode=kv_downsample_mode,
                                                   topk=49, param_attention=param_attention,
                                                   param_routing=param_routing,
                                                   diff_routing=diff_routing, soft_routing=soft_routing,
                                                   side_dwconv=side_dwconv,
                                                   auto_pad=auto_pad, param_size=param_size)
    ...
    def forward(self, x):
        ...
        if self.pre_norm:
            if self.use_layer_scale:
                ...
                x = x + self.drop_path1(
                    self.mlp1(self.norm2(x).permute(0, 3, 1, 2)).permute(0, 2, 3, 1))  # (N, H, W, C)
