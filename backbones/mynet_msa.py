...
from mmseg.models.utils.MSPA import MSAModule
...
class ConvNeXtBlock(BaseModule):
    ...
    def __init__(self,
        ...
        # 集成 MSAModule
        # self.msa = MSAModule(inplanes=in_channels, scale=3, stride=1, stype='normal')
...
class MyNetMSA(BaseModule):
    ...
    def __init__(self,
        ...
        stem = nn.Sequential(
            ...
            MSAModule(inplanes=self.channels[0], scale=3, stride=1, stype='normal')
        )
            ...
            if i >= 1:
                downsample_layer = nn.Sequential(
                    ...
                    nn.Conv2d(
                        ...
                        kernel_size=2,
                        stride=2),
                    MSAModule(inplanes=channels, scale=3, stride=1, stype='normal')
                )
    ...
    def init_weights(self):
        # 1. 调用基类的 init_weights（应用 init_cfg 的初始化）
        super().init_weights()
        parent_map = build_module_parent_map(self)
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                # 检查模块是否位于 MSAModule 内部
                module = m
                in_msa = False
                while module in parent_map:
                    parent = parent_map[module]
                    if isinstance(parent, MSAModule):
                        in_msa = True
                        break
                    module = parent
                if in_msa:
                    nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
                    print(f"Initialized Kaiming for {m}")
                else:
                    # 对其他 Conv2d 层应用默认的初始化方法（根据您的需要）
                    pass
    ...
    def train(self, mode=True):
        super(MyNetMSA, self).train(mode)
        self._freeze_stages()
...
def build_module_parent_map(model):
    parent_map = {}
    def helper(module, parent=None):
        for child_name, child_module in module.named_children():
            parent_map[child_module] = module
            helper(child_module, child_module)
    helper(model)
    return parent_map
