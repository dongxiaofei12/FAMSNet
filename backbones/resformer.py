...
class ResFormer(BaseModule):
    ...
    def __init__(self,
        ...
        for channels in dims:
            self.fusion_modules.append(FeatureFusionModule(channels, channels))
    ...
    def init_weights(self):
        ...
        for name, m in self.named_modules():
            if isinstance(m, nn.Conv2d):
                # 检查模块名称是否包含 'stage1' 到 'stage4'
                if any(stage in name for stage in ['stage1', 'stage2', 'stage3', 'stage4']):
                    # 对 stage1 到 stage4 的 Conv2d 层应用 Kaiming 初始化
                    nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
                    print(f"Initialized Kaiming for {name}")
                else:
                    pass
    ...
    def forward(self, x):
        ...
        for i in range(len(outs)):
            fused = self.fusion_modules[i](features[i], outs[i])
